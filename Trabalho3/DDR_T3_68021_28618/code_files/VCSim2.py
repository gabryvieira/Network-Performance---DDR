...
S_listBlock =[]
S_listAvgCap =[]
listLamb_standard = [1,2,3,4,5]
listLamb_special = [1,1.5,2,2.5,3]
for val in listLamb_special:
	lamb= val
	invmu=10
	b=4
	B=16
	...
	T_listAvgCap = T_listAvgCap +[100*linkload]

plt.figure(1)
plt.subplot(2,1,1)	
plt.plot(listLamb_special,S_listBlock,'blue', label = 'Probability of block')
plt.plot(listLamb_special,T_listBlock, 'r--')
plt.ylabel('P block')
plt.subplot(2,1,2)
plt.plot(listLamb_special,S_listAvgCap,'blue', label = 'Avg capability')
plt.plot(listLamb_special,T_listAvgCap,'r--')
plt.ylabel('AVG cap')
plt.show()
