...
listAvg10k=[]
listTB10k= []
listQueueS=[64.0, 128.0]
listLamb = [150.0, 450.0]
kleinrock64 = 0
kleinrock128 = 0
...
for lamb in listLamb:
	...
	for queueS in listQueueS:
		...
		link1=Link(env,'L1',2e6,np.inf)
		link2 = Link(env, 'L2', 2e6, queueS)
		tx.out=link1
		link1.out = node1
		...
		env.run(simtime)
		if(queueS ==64):
			listLoss64 += [(1-(1.0*rx.packets_recv/(1.0*tx.packets_sent)))*100.0];
			kleinrock64 = (1.0*link1.lost_pkts + 1.0*node1.lost_pkts + 1.0*link2.lost_pkts)/(1.0*tx.packets_sent)
			#listLoss64 += [((100*link1.lost_pkts/tx.packets_sent*1.0)+(100*((link2.lost_pkts/(tx.packets_sent -link1.lost_pkts) )*1.0)))]
			listAvg64 += [1.0*rx.overalldelay/rx.packets_recv]
			listTB64 += [1.0*rx.overallbytes/simtime]
		#if(queueS ==96):
			#listLoss96 += [100*((link.lost_pkts/tx.packets_sent))*1.0+100*( (link2.lost_pkts/(tx.packets_sent -link.lost_pkts) )*1.0)]
			#listAvg96 += [1.0*rx.overalldelay/rx.packets_recv]
			#listTB96 += [1.0*rx.overallbytes/simtime]
		if(queueS ==128):
			listLoss128 += [(1-(1.0*rx.packets_recv/(1.0*tx.packets_sent)))*100.0]
			kleinrock128 = (1.0*link1.lost_pkts + 1.0*node1.lost_pkts + 1.0*link2.lost_pkts)/(1.0*tx.packets_sent)
			#listLoss128 += [((100*link1.lost_pkts/tx.packets_sent*1.0)+(100*((link2.lost_pkts/(tx.packets_sent -link1.lost_pkts) )*1.0)))]
			listAvg128 += [1.0*rx.overalldelay/rx.packets_recv]
			listTB128 += [1.0*rx.overallbytes/simtime]
		#if(queueS ==10000):
			#listLoss10k += [100*((link.lost_pkts/tx.packets_sent))*1.0+100*( (link2.lost_pkts/(tx.packets_sent -link.lost_pkts) )*1.0)]
			#listAvg10k += [1.0*rx.overalldelay/rx.packets_recv]
			#listTB10k += [1.0*rx.overallbytes/simtime]
		print('Loss probability for 64 queue size')
		print(listLoss64)
		print('Loss probability for 128 queue size')
		print(listLoss128)
		print('kleinrock 64: %.9f'% kleinrock64)
		print('kleinrock 128: %.9f'% kleinrock128)
		print('Loss probability: %.2f%%'%((1-(1.0*rx.packets_recv/(1.0*tx.packets_sent)))*100.0))
