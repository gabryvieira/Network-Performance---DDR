...
for lamb in listLamb:
	...
	for queueS in listQueueS:
		...
		node2 = Node(env,'N2',350, queueS)
		link1=Link(env,'L1',10e9,np.inf)
		link2 = Link(env, 'L2', 10e9, np.inf)
		tx.out=link1
		link1.out = node1
		...
		if(queueS ==64):
			...
			kleinrock64 = (1.0*node1.lost_pkts + 1.0*node2.lost_pkts)/(1.0*tx.packets_sent)
			#listLoss64 += [((100*link.lost_pkts/tx.packets_sent*1.0)+(100*((link2.lost_pkts/(tx.packets_sent -link.lost_pkts) )*1.0)))]
		...
		if(queueS ==128):
			...
			kleinrock128 = (1.0*node1.lost_pkts + 1.0*node2.lost_pkts)/(1.0*tx.packets_sent)
