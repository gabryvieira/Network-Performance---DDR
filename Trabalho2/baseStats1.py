...
data2=np.loadtxt('data2') # youtube packets
#data2=np.loadtxt('downBytesPcap') # youtube packets

#for i in range(0,39):
	#plt.figure(1)
	#plt.plot(data1[:,0],marker='s',c='blue',label='dataset 0') # para a amostra n
	#plt.show()
...
#plt.plot(M)
#plt.ylabel('mean')
#plt.subplot(5,1,2)  # media
#plt.plot(Md)
#plt.ylabel('median') # mediana
#plt.subplot(5,1,3) 
#plt.plot(V)
#plt.ylabel('variance') # variancia
#plt.subplot(5,1,4) 
#plt.plot(S)
#plt.ylabel('skewness') # skewness
#plt.subplot(5,1,5) 
#plt.plot(K)
#plt.ylabel('kurtosis') # kurtosis
#plt.show()

#media = 0
## code added to exercise 3
...
#p2=range(5,101,1)
#Pr2=np.percentile(data2,p2,axis=0)
#print("Youtube metrics")
#print
#print("media")
#print(M2)
#print("mediana")
...
#usando dataPcapDown obtem se os seguintes valores
#media
#1.35570469799
#mediana
#0.0
...
for a in range(0,19):
	pdf, bins = np.histogram(data1[:,a],bins=50,density=True)
	dbin=np.diff(bins)[a]
...
for b in range(20,39):
	pdf, bins = np.histogram(data1[:,b],bins=50,density=True)
	dbin=np.diff(bins)[b]
	cdf=np.cumsum(pdf)*dbin
	x=bins[:-1]		
	#plt.figure(4)	
...
#plt.show()
 ...
 ##calculo da distancia de hellinger
#hell_dist = hellingerDist(pdf0, pdf1)
#print("Distancia de hellinger: ")
#print(hell_dist)
#waitforEnter()
...
#KS_a = stats.ks_2samp(data1[:,0],data1[:,1])
##for dataset 0 and dataset 30
#KS_b = stats.ks_2samp(data1[:,0],data1[:,30])
#print(KS_a)
#print(KS_b)
...
He, binsE = np.histogram(ye, bins=20)
#Gaussian/Normal
yg=np.random.normal(M,V**0.5,(300,100))
Hg, binsG = np.histogram(yg, bins=100)
...
plt.figure(17)
plt.plot(wx, H, marker='s',c='red',label='dataset 0 PDF')
plt.show()
plt.figure(18)
plt.plot(w_ye, He, marker='s',c='red',label='dataset 0 PDF')
plt.show()
plt.figure(19)
plt.plot(w_yg, Hg, marker='s',c='red',label='dataset 0 PDF')
plt.show()
#p=range(6,101,1)
#plt.subplot(6,1,1) 
#plt.plot(x)
...
#plt.show()
#pdf, bins = np.histogram(data1[:,0],bins=20)
#dbin=np.diff(bins)[0]
#cdf=np.cumsum(pdf)*dbin
#x=bins[:-1]	
#plt.subplot(6,1,5)
