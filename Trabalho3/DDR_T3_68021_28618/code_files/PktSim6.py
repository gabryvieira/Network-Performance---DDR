...
listAvg256=[]
listTB256= []
listQueueS=[64.0,256.0]
listLamb = [150.0,600,0]
for lamb in listLamb:
	...
	for queueS in listQueueS:
		...
		node1 = Node(env,'N1',500,queueS)
		node2 = Node(env,'N2',500,queueS)
		node3 = Node(env,'N3',500,queueS)
		node4 = Node(env,'N4',500,queueS)
		...
		env.run(simtime)
		if(queueS ==64):
			#listLoss64 =[100.0*((1.0*node1.lost_pkts/txA.packets_sent)+(1.0*node2.lost_pkts/(txB.packets_sent)) + (1.0*link1.lost_pkts/(txA.packets_sent - node1.lost_pkts))+(1.0*link2.lost_pkts/(txB.packets_sent - node2.lost_pkts))+(1.0*node3.lost_pkts/(txA.packets_sent - node1.lost_pkts-link1.lost_pkts + txB.packets_sent - node2.lost_pkts-link2.lost_pkts))+(1.0*link3.lost_pkts/(txA.packets_sent - node1.lost_pkts-link1.lost_pkts-node3.lost_pkts +txB.packets_sent - node2.lost_pkts-link2.lost_pkts))+(1.0*node4.lost_pkts/(txA.packets_sent - node1.lost_pkts-link1.lost_pkts-node3.lost_pkts-link3.lost_pkts + txB.packets_sent - node2.lost_pkts-link2.lost_pkts))+(1.0*link4.lost_pkts/(txA.packets_sent - node1.lost_pkts-link1.lost_pkts-node3.lost_pkts-link3.lost_pkts-node4.lost_pkts+ txB.packets_sent - node2.lost_pkts-link2.lost_pkts)))]
			listLoss64 +=[(1-(1.0*rx.packets_recv/(1.0*txA.packets_sent+1.0*txB.packets_sent)))*100.0]
			listAvg64 += [1.0*rx.overalldelay/1.0*rx.packets_recv]
			listTB64 += [1.0*rx.overallbytes/simtime]
		#if(queueS ==96):
			#listLoss96 += [(1-(1.0*rx.packets_recv/(1.0*txA.packets_sent+1.0*txB.packets_sent)))*100.0]
			#listAvg96 += [1.0*rx.overalldelay/rx.packets_recv]
			#listTB96 += [1.0*rx.overallbytes/simtime]
		#if(queueS ==128):
			#listLoss128 += [(1-(1.0*rx.packets_recv/(1.0*txA.packets_sent+1.0*txB.packets_sent)))*100.0]
			#listAvg128 += [1.0*rx.overalldelay/rx.packets_recv]
			#listTB128 += [1.0*rx.overallbytes/simtime]
		if(queueS ==256):
			listLoss256 += [(1-(1.0*rx.packets_recv/(1.0*txA.packets_sent+1.0*txB.packets_sent)))*100.0]
			listAvg256 += [1.0*rx.overalldelay/1.0*rx.packets_recv]
			listTB256 += [1.0*rx.overallbytes/simtime]
		print(rx.packets_recv)
		print((txA.packets_sent+txB.packets_sent))
		print('Loss probability: %.2f%%'%((1-(1.0*rx.packets_recv/(1.0*txA.packets_sent+1.0*txB.packets_sent)))*100.0))
		print('Average delay: %f sec'%(1.0*rx.overalldelay/1.0*rx.packets_recv))
